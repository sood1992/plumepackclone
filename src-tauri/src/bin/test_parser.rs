...
//! Run with: cargo run --bin test_parser -- <path_to_prproj>

use std::env;
use std::path::PathBuf;

// Include the parser module inline since we can't easily import it
...
fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        eprintln!("Usage: test_parser <path_to_prproj>");
        eprintln!("Example: test_parser /path/to/project.prproj");
        std::process::exit(1);
    }
    ...
    // Parse the XML
    parse_xml(&xml_content, &prproj_path)?;
...
fn parse_xml(xml_content: &str, file_path: &Path) -> Result<()> {
